...
describe("trace_name ngram-index prefilter", () => {
  ...
  it("prepends an OR-of-equalities prefilter for `any of`", () => {
    const { query } = traceNameOptions("any of", ["foo", "bar"]).apply();
    expect(query).toContain("lower(e.trace_name) = lower({");
  ...
  it("leaves bare-column string filters untouched", () => {
    // `name`/`user_id`/`session_id` acceleration is a separate concern; only the
    // trace_name COALESCE expression is rewritten here.
    ...
    expect(query).toMatch(
      /^position\(e\.name, \{stringFilter\w+: String\}\) > 0$/,
    );
  });
});
